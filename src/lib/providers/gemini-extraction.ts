...
export async function extractEntitiesWithGemini(sourceText: string): Promise<GeminiExtractionResult> {
  ...
  try {
    ...
    if (!response.ok) throw new GeminiExtractionError('UNAVAILABLE', `Gemini request failed with status ${response.status}`);
    ...
    throw new GeminiExtractionError('UNAVAILABLE', error instanceof Error && error.name === 'AbortError'
