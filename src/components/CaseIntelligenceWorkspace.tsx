          ...
          {search.findings.length > 0 ? (
            <div className="space-y-3" aria-label="รายการข้อมูลที่เกี่ยวข้อง">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div><h3 className="flex items-center gap-2 text-base font-black text-white"><FileSearch className="h-5 w-5 text-indigo-300" />ข้อมูลที่ระบบพบ</h3><p className="mt-1 text-[10px] text-slate-500">เรียงเป็นสัญญาณข้อมูล พร้อมหลักฐานและ citation ทุกชิ้น</p></div>
                <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-300/15 bg-emerald-300/[0.06] px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.12em] text-emerald-200"><ShieldCheck className="h-3.5 w-3.5" /> Source trace 100%</span>
              </div>
                ...
                {search.findings.map((finding, index) => {
                  ...
                  <article key={finding.id} className={`group relative overflow-hidden rounded-3xl border p-5 transition hover:-translate-y-0.5 hover:border-cyan-200/25 hover:shadow-[0_16px_44px_rgba(2,8,23,0.35)] motion-reduce:transition-none ${findingStyle[finding.kind]}`}>
                    ...
                    <div className="mt-4 border-t border-white/[0.08] pt-4 text-[10px] text-slate-400">
                      <div className="flex items-start gap-2"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-300" /><div><p className="font-mono text-[9px] uppercase tracking-[0.12em] text-slate-600">Verified source</p><p className="mt-1 font-semibold text-slate-200">{finding.source.label}</p>{finding.source.pageNumber && <p className="mt-1">หน้า {finding.source.pageNumber}{finding.source.sha256 ? ` · SHA-256 ${finding.source.sha256.slice(0, 12)}…` : ''}</p>}{finding.source.publishedDate && <p className="mt-1">วันที่ในทะเบียน: {finding.source.publishedDate}</p>}</div></div>
                      <div className="mt-3">
                        {finding.source.url
