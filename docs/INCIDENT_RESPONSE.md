...
3. Preserve Cloudflare deployment/version identifiers, Supabase logs, audit events, object hashes, file-validation metadata, and relevant request IDs.
...
- **SEV-2:** suspected unauthorized access, failed integrity check, material upload outage, or isolated privilege defect.
...
- File-validation failure: uploads remain unfinalized; pause intake if stored objects cannot be verified; never mark an unscanned object CLEAN.
...
Recovery requires health status `ready`, clean release gates, confirmed secret rotation, role-boundary tests, stored-object validation, evidence hash verification, and approval by the incident commander plus Security. Complete root cause, affected-data assessment, notification decision, corrective actions, owners, due dates, and a post-incident review.
